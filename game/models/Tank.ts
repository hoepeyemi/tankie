...
export default class Tank extends Entity {
	...
	public get kills() {
		return this.properties.getProperty('kills').value;
	}

	public set kills(value: number) {
		this.properties.getProperty('kills').value = value;
	}

	public get deaths() {
		return this.properties.getProperty('deaths').value;
	}

	public set deaths(value: number) {
		this.properties.getProperty('deaths').value = value;
	}

	public get xp() {
		return this.properties.getProperty('xp').value;
	}

	public set xp(value: number) {
		this.properties.getProperty('xp').value = value;
	}
	...
	public getDistanceTo(tank: Tank): { distance: number; direction: number } {
        ...
        // Compass needs to be relative to the player's camera perspective, not the spinning turret or the rotating chassis.
		const cameraDirection = this.game.camera.getWorldDirection(new THREE.Vector3());
		const direction = Math.atan2(cameraDirection.x, cameraDirection.z)
	...
	public async teleport(position: THREE.Vector3) {
