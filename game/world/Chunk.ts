...
export class Chunk extends ExtendedGroup {
	...
	public get physicsMesh(): ExtendedMesh {
		if (!this._physicsMesh) {
			...
			const mesh = new ExtendedMesh(geometry.clone());
	...
	private makeWater() {
		const geometry = new THREE.PlaneGeometry(Chunk.chunkSize, Chunk.chunkSize, 32, 32);
