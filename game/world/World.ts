...
export class World {
    ...
    public async getSpawnPositionNear(targetPos: Vector3): Promise<Vector3> {
        // Spawn randomly within an annulus (donut shape) around the target
        // between 30 and 60 units away to ensure they are inside the visible fog (max 120m)
        const angle = Math.random() * Math.PI * 2;
        const distance = 30 + (Math.random() * 30);
        
        const spawnX = targetPos.x + (Math.cos(angle) * distance);
        const spawnZ = targetPos.z + (Math.sin(angle) * distance);

        return this.getPosition(spawnX, spawnZ);
    }
	...
	public async update() {
		...
		this.chunks.forEach((chunk, chunkId) => {
			...
			if (chunk.value.lastUpdate + 60000 < Date.now()) {
				chunk.value.destroy();
