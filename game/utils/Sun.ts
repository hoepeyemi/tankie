...
export class SkyBox extends THREE.Group {
	...
	private createMesh(texture: THREE.Texture, z = 0, materialOptions: Partial<THREE.MeshBasicMaterialParameters> = {}) {
		return new THREE.Mesh(
			new THREE.SphereGeometry(1000 + z, 32, 32),
...
export class Sun extends DirectionalLight {
	...
	private readonly _colorA = new THREE.Color('#adc1d8');
	private readonly _colorB = new THREE.Color('#002e5a');
	private readonly _lerpColor = new THREE.Color();
	...
	public update() {
		...
		this._lerpColor.copy(this._colorA).lerp(this._colorB, 1 - (y / distance));
		this.scene.scene.background = this._lerpColor;
		this.scene.scene.fog!.color = this._lerpColor;
